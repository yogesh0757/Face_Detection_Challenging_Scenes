...
parser.add_argument('-m', '--trained_model', default='./weights/LiteNet_epoch_130.pth',
...
parser.add_argument('--confidence_threshold', default=0.03, type=float, help='confidence_threshold')
...
if __name__ == '__main__':
    ...
    for i, img_name in enumerate(test_dataset):
        ...
        _t['forward_pass'].tic()
        loc, conf, conf_cond, landms = net(img)  # forward pass
        _t['forward_pass'].toc()
        _t['misc'].tic()
        ...
        scores_cond = conf_cond.squeeze(0).data.cpu().numpy()[:, 1]
        scores = (scores + scores_cond)/2
